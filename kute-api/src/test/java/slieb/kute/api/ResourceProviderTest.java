...
public final class ResourceProviderTest {
    ...
    @Before
    public void setup() {
        resourceA = readableResource("/path/a", "A");
        resourceB = readableResource("/path/b", "B");
        resourceC = readableResource("/path/c", "C");
    }
    ...
    public void testProviderChecksumMD5() throws IOException, NoSuchAlgorithmException {
        Resource.Provider provider = () -> Stream.of(TestUtils.readableResource(TestUtils.EXPECTED_PATH, EXPECTED_CONTENT));
        assertEquals(EXPECTED_MD5_CHECKSUM, encode(provider.checksum("MD5")));
    }

    @Test
    public void testProviderChecksumSHA() throws IOException, NoSuchAlgorithmException {
        Resource.Provider provider = () -> Stream.of(TestUtils.readableResource(TestUtils.EXPECTED_PATH, EXPECTED_CONTENT));
        assertEquals(EXPECTED_SHA_CHECKSUM, encode(provider.checksum("SHA")));
    }
}
