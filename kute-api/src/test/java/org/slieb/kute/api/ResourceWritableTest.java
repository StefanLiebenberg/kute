...
public class ResourceWritableTest {
    ...
    @Before
    public void setup() {
        writable = TestUtils.writableResource(TestUtils.EXPECTED_PATH, reference);
    }
    ...
    public void testResourceWritableGetDefaultWriter() throws IOException {
        ...
        try (Writer writer = writable.getWriter()) {
            ...
            IOUtils.write(TestUtils.EXPECTED_CONTENT, writer);
        }
        Assert.assertEquals(TestUtils.EXPECTED_CONTENT, reference.get().toString());
        Assert.assertEquals(TestUtils.EXPECTED_PATH, writable.getPath());
    }
    ...
    public void testResourceWritableUseOutputStream() throws IOException {
        writable.useOutputStream(outputStream -> {
            ...
            IOUtils.write(TestUtils.EXPECTED_CONTENT, outputStream);
        });
        Assert.assertEquals(TestUtils.EXPECTED_CONTENT, reference.get().toString());
        Assert.assertEquals(TestUtils.EXPECTED_PATH, writable.getPath());
    }
